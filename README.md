...
- **Older history in the background.** **Settings > Chats > Download older
  ...
  scrolling up does not run into the phone's rate limit. **Recent and pinned**
  (the default) covers every pinned chat and the ten most recently active
  ones, **Current chat** covers only the open one, and **Off** fetches nothing
  in the background. The open chat goes first. A chat the phone says it has no
  more of is left alone until the link reconnects, and a background request
  never moves the view or interrupts one you asked for. A file that fails to
  download is tried again with a growing delay (30 seconds, doubling to 15
  minutes, then hourly) for thirty days; the bubble says what is happening and
  clicking it opens a fresh window.
